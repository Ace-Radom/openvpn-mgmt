...
class clients:
    ...
    def __init__(self):
        self._client_certs_list_file = os.path.join(
            settings.settings["server"]["server_dir"], "easy-rsa/pki/index.txt"
        )
        self._client_ipp_file = os.path.join(
            settings.settings["server"]["server_dir"], "ipp.txt"
        )
    ...
    def refresh_client_data(self) -> int:
        ...
        if not os.path.isfile(self._client_certs_list_file):
            raise Exception(
                f'Valid clients list file missing: "{ self._client_certs_list_file }"'
            )

        if not os.path.isfile(self._client_ipp_file):
            raise Exception(
                f'Valid clients ipp file missing: "{ self._client_ipp_file }"'
            )
        ...
        for client in detailed_valid_clients:
            if len(self._client_data) != 0 and any(
                client["common_name"] == client_data["common_name"]
                for client_data in self._client_data
            ):
                client_data = list(
                    filter(
                        lambda it: it["common_name"] == client["common_name"],
                        self._client_data,
                    )
                )
                this_client_data = {
            ...
            if (
                this_client_data["common_name"]
                in settings.settings["clients"]["admins"]
            ):
                this_client_data["user_group"] = self.USER_ADMIN
                this_client_data["block_to"] = -1
            elif (
                this_client_data["common_name"]
                in settings.settings["clients"]["blocked_users"]
            ):
        ...
        log.logger.write_log(
            self._loghost,
            f"Clients data refreshed. [client_count={ len( self._client_data ) }]",
        )
        log.logger.write_log(
            self._loghost, f"New client data: { json.dumps( new_client_data ) }"
        )
        return 0
            ...
            # no need to update admin users' & blocked users' block_to ts
                ...
                # this normal user is not blocked or indefinitely blocked
                    ...
                    if block_to < ts_now:
                        log.logger.write_log(
                            self._loghost,
                            f"Client is no longer blocked. [cn=\"{ self._client_data[i]['common_name'] }\", block_to={ block_to }]",
                        )
    ...
    def list_client(self) -> int:
        ...
        if len(self._client_data) == 0:
            utils.lprint(
                2,
                'Clients data empty, run "clients --refresh" first to refresh cached clients data.',
            )
            return 1

        clients_list = []
        clients_list.append(["Common Name", "Virtual IP", "User Group"])
        ...
        for client in self._client_data:
            clients_list.append(
                [
                    ...
                    client["virtual_ip"],
                    {
                        clients.USER_ADMIN: "Admin",
                        clients.USER_NORMAL: "Normal User",
                        clients.USER_BLOCKED: "Blocked User",
                    }.get(client["user_group"], "Unknown"),
                ]
            )
    ...
    def list_client_status(self) -> int:
        ...
        if settings.settings["base"]["use_mgmt_interface_as_default"]:
            ...
            if ret == 0:
                ...
                if ret != 0:
                    log.logger.write_log(
                        self._loghost,
                        "Get service status log from OpenVPN management interface failed, use status log file instead.",
                    )
                mgmt_interface.close()
            else:
                log.logger.write_log(
                    self._loghost,
                    "Connect to OpenVPN management interface failed, use status log file instead.",
                )

        if ret != 0 or status_log == "":
            with open(
                settings.settings["server"]["status_log"], "r", encoding="utf-8"
            ) as rFile:
        ...
        clients_list = []
        clients_list.append(
            ["Common Name", "Virtual IP", "User Group", "Status", "Additional Info"]
        )
                ...
                if ret == 0:
                    if tag == "yes":
                        ...
                        if status.is_connected(client["common_name"]):
                            connected_client_data = list(
                                filter(
                                    lambda it: it["common_name"]
                                    == client["common_name"],
                                    status.detailed_connection_data,
                                )
                            )
                            assert len(connected_client_data) == 1
                            client_connected_since_time_str = (
                                datetime.datetime.fromtimestamp(
                                    connected_client_data[0]["connected_since"]
        ...
        last_log_refresh_time_str = status.last_log_refresh_datetime.strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        utils.lprint(
            1,
            f"Last log refresh: { last_log_refresh_time_str } { utils.get_tzname() } ({ last_log_refresh_time_delta_str } seconds ago)",
        )
    ...
    def block_client(self, common_name: str, block_time_length_str: str) -> int:
        ...
        for client in self._client_data:
            if client["common_name"] == common_name:
                ...
                utils.lprint(
                    1,
                    f"Client '{ common_name }' has been blocked to { block_to.strftime( '%Y-%m-%d %H:%M:%S' ) } { datetime.datetime.now().astimezone().tzname() }.",
                )
                log.logger.write_log(
                    self._loghost,
                    f"Client blocked. [cn='{ common_name }', block_to={ block_to_ts }]",
                )
                return 0

        utils.lprint(
            2,
            f'Client "{ common_name }" not found in cached client data. You may try refreshing cached client data first.',
        )
        return 1
    ...
    def _is_client_blocked(self, common_name: str) -> tuple[int, str, str]:
        ...
        client_data = list(
            filter(lambda it: it["common_name"] == common_name, self._client_data)
        )
        if len(client_data) == 0:
            return (
                1,
                "error",
                f'Client "{ common_name }" not found in cached client data.',
            )
            ...
            if block_to == -1:
                ...
                if ts_now < block_to:
                    return (
                        0,
                        "yes",
                        f"Client \"{ common_name }\" is blocked to { datetime.datetime.fromtimestamp( block_to ).strftime( '%Y-%m-%d %H:%M:%S' ) } { datetime.datetime.now().astimezone().tzname() }.",
                    )
                else:
