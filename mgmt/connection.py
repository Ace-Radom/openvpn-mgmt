...
class connection:
    ...
    CONNECTION_DATA_DEFAULT = {"mode": CONNECTION_MODE_NORMAL}
    ...
    def set_mode(self, mode: int) -> int:
        if mode not in [
            connection.CONNECTION_MODE_NORMAL,
            connection.CONNECTION_MODE_MAINTAIN,
            connection.CONNECTION_MODE_BLOCK,
        ]:
        ...
        self.write_data()
        log.logger.write_log(
            self._loghost, f"Connection mode changed. [new_mode={ mode }]"
        )
        return 0
    ...
    def list_connection_datas(self) -> int:
        ...
        last_log_refresh_time_str = (
            self._ovpn_status.last_log_refresh_datetime.strftime("%Y-%m-%d %H:%M:%S")
        )
        ...
        utils.lprint(
            1,
            f"Current connected clients count: { self._ovpn_status.connection_count }",
        )
        ...
        col_widths = [
            ...
            for col in zip(*detailed_connection_data_list)
        ]
    ...
    def kill_connection(self, common_name: str) -> int:
        if (
            settings.settings["server"]["mgmt_interface_server"] is None
            or settings.settings["server"]["mgmt_interface_port"] is None
        ):
            utils.lprint(
                2,
                f"Cannot kill client connection without OpenVPN management interface setup",
            )
            return 1
        ...
        if not self._ovpn_status.is_connected(common_name):
            utils.lprint(
                2, f'Client "{ common_name }" isn\'t connected, cannot kill connection'
            )
            return 1
        ...
        ret = mgmt_interface.connect()
        if ret == 0:
            ret, response = mgmt_interface.exec(
                f"kill { common_name }", end_sign="SUCCESS"
            )
            if ret == 0:
                log.logger.write_log(
                    self._loghost,
                    f"Client connection killed. [cn='{ common_name }', response='{ response }']",
                )
            else:
                log.logger.write_log(
                    self._loghost,
                    f"Kill client connection failed. [cn='{ common_name }', response='{ response }']",
                )
            mgmt_interface.close()
        else:
            log.logger.write_log(
                self._loghost,
                f"Connect to OpenVPN management interface failed, cannot kill client connection. [cn='{ common_name }']",
            )
