...
class ovpn_script:
    def __init__(self, base_dir: str):
        ...
        for script in scripts:
            ...
            if not os.path.isfile(os.path.join(self._script_dir, script_path)):
                continue
            self._scripts.append({"script": script, "path": script_path})
    ...
    def install(self, target: str) -> int:
        script_install_dir = os.path.join(
            settings.settings["server"]["server_dir"], "script"
        )
        ovpn_cfg_setup_info = ""
        copied = False
        ...
        for script in self._scripts:
            ...
            try:
                ...
                with open(to_path, "r+", encoding="utf-8") as File:
                    ...
                    content = content.replace(
                        "$OPENVPN_MGMT_MAIN_PATH",
                        os.path.join(self._base_dir, "mgmt.py"),
                    )
                    File.seek(0)
                    File.write(content)
                    File.truncate()
                ...
                log.logger.write_log(
                    self._loghost,
                    f"Failed to install OpenVPN script. [type='{ script['script'] }', from='{ from_path }', to='{ to_path }', error='{ e }']",
                )
            ...
            log.logger.write_log(
                self._loghost,
                f"OpenVPN script installed. [type='{ script['script'] }', from='{ from_path }', to='{ to_path }']",
            )
            utils.lprint(1, f" - { from_path } -> { to_path }: Succeeded")
            ovpn_cfg_setup_info += (
                f" - { script['script'] } \"script/{ script['path'] }\"\n"
            )

        if not copied:
            utils.lprint(
                2,
                f'Target "{ target }" doesn\'t exist or none of the installation succeeded.',
            )
            return 1

        ovpn_cfg_setup_info = (
            "OpenVPN script installation finished.\nYou may add the following lines into your server config file:\n"
            + ovpn_cfg_setup_info
        )
        ovpn_cfg_setup_info += (
