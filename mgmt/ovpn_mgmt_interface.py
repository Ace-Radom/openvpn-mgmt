...
class ovpn_mgmt_interface:
    ...
    def __init__(self):
        if (
            ...
            raise Exception(
                "OpenVPN management interface address or port is empty in settings"
            )
        ...
        try:
            ...
            if "ENTER PASSWORD" in response:
                if settings.settings["server"]["mgmt_interface_pswd"] is None:
                    self._sock.close()
                    raise Exception(
                        "Connecting to OpenVPN management interface with password protected but password is not provided in settings"
                    )

                self._sock.sendall(
                    f"{ settings.settings['server']['mgmt_interface_pswd'] }\n".encode(
                        "utf-8"
                    )
                )
                response = self._sock.recv(1024).decode("utf-8")
            ...
            log.logger.write_log(
                self._loghost, "Connected to OpenVPN management interface"
            )
            ...
            log.logger.write_log(
                self._loghost,
                f"Failed to connect to OpenVPN management interface. [error='{ e }']",
            )
            self._sock = None
            return 1
    ...
    def exec(self, cmd: str, end_sign: str = "END") -> tuple[int, str]:
        try:
            if (
                self._sock is None
                or not ovpn_mgmt_interface.ovpn_mgmt_interface_connected
            ):
            ...
            log.logger.write_log(
                self._loghost,
                f"Execute command succeeded. [response='{ response_str }']",
            )
            ...
            log.logger.write_log(
                self._loghost,
                f"Failed to execute command. [cmd='{ cmd }', error='{ e }']",
            )
            return (1, "")
    ...
    def close(self) -> int:
        try:
            ...
            log.logger.write_log(
                self._loghost, "OpenVPN management interface disconnected."
            )
            return 0
            ...
            log.logger.write_log(
                self._loghost,
                f"Failed to disconnect OpenVPN management interface. [error='{ e }']",
            )
            return 1
